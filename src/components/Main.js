...
export default function Main() {
  // time slots to initialize the availableTimes state
  const initializeSlots = ['17:00', '18:00', '19:00', '20:00', '21:00', '22:00']
  ...
  // initialize availableTimes state as a list of time slots from initializeSlots
  const initializeTimes = AvailableSlots(initializeSlots)
  ...
  const updateTimes = (v, { type }) => {
    // make it so time options leaves out 17:00 if today's date is selected and both 17:00 & 18:00 if tomorrow's date is selected
    ...
    if (type === todayFormatted)
      return (['18:00', '19:00', '20:00', '21:00', '22:00'].map(i => <BookingSlot key={i}>{i}</BookingSlot>))
    if (type === tomorrowFormatted)
      return (['19:00', '20:00', '21:00', '22:00'].map(i => <BookingSlot key={i}>{i}</BookingSlot>))
